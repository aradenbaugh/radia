...
def main():
    ...
    # python radiaCompare.py TCGA-AB-2995 12
    # ../data/test/TCGA-AB-2995.vcf
    # ../data/test/TCGA-AB-2995.vcf
    # -c "SOM=Somatic"
    # -s ../stats/radia/cmpRadBB.tab
    # --log=DEBUG
    ...
    i_cmdLineParser.add_option(
        "-c", "--compareList",
        dest="compareList", metavar="COMPARE_LIST",
        help="a comma separated list of key/values comparisons where the " +
             "key is in RADIA and the value is in the compare file")
    i_cmdLineParser.add_option(
        "-s", "--statsFilename",
        dest="statsFilename", metavar="STATS_FILE",
        help="the name of the stats file, sys.stdout by default")
    i_cmdLineParser.add_option(
        "-l", "--log",
    ...
    i_cmdLineParser.add_option(
        "-o", "--overlapFilename",
        dest="overlapFilename", metavar="OVERLAP_FILE",
        help="the name of the overlap file")
    i_cmdLineParser.add_option(
        "-n", "--nonOverlapFilename",
        dest="nonOverlapFilename", metavar="NON_OVERLAP_FILE",
        help="the name of the non-overlap file")
    ...
    if (i_cmdLineOptions.compareList is not None):
    ...
    compare_events(i_id, i_chr, i_radiaFilename, i_compareFilename,
                   i_statsFilename, i_overlapFilename, i_nonOverlapFilename,
                   i_compareDict, i_debug)

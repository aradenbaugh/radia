...
def is_valid_read_blast_format(aBlatHitsList, aVCFChrom, aVCFCoordinate,
    ...
    for blatHit in aBlatHitsList:
        ...
        if ((readPaired and properlyPaired) or (not readPaired)):
            ...
            if (((("chr" + aVCFChrom) == blatChrom) or
                 (aVCFChrom == blatChrom)) and
                (aVCFCoordinate >= blatRefStart) and
                (aVCFCoordinate <= blatRefStop)):
                # if the evalue is less than the min, then move
                # the old min to the list and set the new min
                if (blatEValue < minOverlapEValue):
                    otherEValues.append(minOverlapEValue)
                ...
                # size and the percent identity is greater than the current
                # max, then move the old max to the list and set the new max
                if (blatAlignmentLength > readLengthHalf and
                    blatIdentity > maxOverlapIdentity):
                    otherIdentities.append(maxOverlapIdentity)
            ...
            # print "min=", minOverlapEValue, "others=", otherEValues
    ...
    elif (minOverlapEValue == sys.float_info.max):
        return (False, validRead)
    ...
    if (anOrderMagnitude == 0):
        ...
        '''
        if (minOtherEvalue < (minOverlapEValue/(1/(10.0**anOrderMagnitude)))
            and (maxOtherIdentity > maxOverlapIdentity)):
        '''
    ...
    '''
    # if another read had a better evalue (within a certain order of
    # magnitude), then return false, otherwise our read is mapped to
    # the best spot in the genome
    minOther = min(otherEValues)
    if (anOrderMagnitude == 0):
        if (minOther < minOverlapEValue):
            return (False, validRead)
        else:
            return (True, validRead)
    else:
        if (minOther < (minOverlapEValue/(1/(10.0**anOrderMagnitude)))):
            return (False, validRead)
        else:
            return (True, validRead)
    '''
...
def is_valid_read_psl_format(aBlatHitsList, aVCFChrom,
                             aVCFCoordinate, anIsDebug):
    '''
    ' This method determines if the read is valid.  It compares all of the
    ' BLAT hits to determine if the read mapped to another location in the
    ' genome with a better score.
    ...
    for blatHit in aBlatHitsList:
        ...
        if ((readPaired and properlyPaired) or (not readPaired)):
            ...
            '''
            print (blatScore, matches, repeatMatches,
                   mismatches, qNumInserts, tNumInserts)
            '''
            # if the blat hit covers the coordinate that we're investigating
            if ((("chr" + aVCFChrom) == tName) and
                (aVCFCoordinate >= tStart) and
                (aVCFCoordinate <= tEnd)):
                # if the score is greater than the max, then move the old max
                # to the list and set the new max
...
def main():
    ...
    i_cmdLineParser.add_option(
        "-d", "--readDepthCutoff", type="int", default=int(4),
        dest="readDepthCutoff", metavar="READ_DP_CUTOFF",
        help="the minimum number of valid reads that are necessary, " +
             "%default by default")
    i_cmdLineParser.add_option(
        "-p", "--readPercentCutoff", type="float", default=float(0.10),
        dest="readPercentCutoff", metavar="READ_PERCENT_CUTOFF",
        help="the minimum percentage of valid reads that are necessary, " +
             "%default by default")

    '''
    i_cmdLineParser.add_option(
        "-e", "--eValueCutoff", type="float", default=float(10e-6),
        dest="eValueCutoff", metavar="EVAL_CUTOFF",
        help="the e-value cutoff for determining if a blat hit is " +
             "significant, %default by default")
    i_cmdLineParser.add_option(
        "-u", "--upperIdentityCutoff", type="float", default=float(0.95),
        dest="upperIdentityCutoff", metavar="UPPER_CUTOFF",
        help="the upper cutoff for the match length adjusted identity to " +
             "determine if a blat hit is significant, %default by default")
    i_cmdLineParser.add_option(
        "-l", "--lowerIdentityCutoff", type="float", default=float(0.5),
        dest="lowerIdentityCutoff", metavar="LOWER_CUTOFF",
        help="the lower cutoff for the match length adjusted identity to " +
             "determine if a second blat hit is significant, " +
             "%default by default")
    '''
    ...
    if (i_cmdLineOptions.logFilename is not None):
        i_logFilename = i_cmdLineOptions.logFilename
    ...
    i_writeFilenameList = []
    if (i_cmdLineOptions.outputFilename is not sys.stdout):
    ...
    for (vcfChr, vcfStopCoordinate, vcfId, vcfRef, vcfAlt, vcfScore,
        ...
        if (i_debug):
            logging.debug("VCF Data: %s %s %s %s %s %s %s %s %s",
                          vcfChr, str(vcfStopCoordinate), vcfId,
        ...
        for modType in modTypes:
            ...
            numValidReads = 0
            vcfKey = vcfChr + "_" + str(vcfStopCoordinate)
            if (modType == "NOR_EDIT" and i_blatRnaNormalReads):
                if ("rnaNormal" in i_blatCoordinateDict[vcfKey]):
                    # for each coordinate, get a dict of reads
                    # and corresponding blat hits
                    blatHitsDict = i_blatCoordinateDict[vcfKey]["rnaNormal"]
            elif ((modType == "SOM" or modType == "TUM_EDIT") and
                  i_blatRnaTumorReads):
                if ("rnaTumor" in i_blatCoordinateDict[vcfKey]):
                    # for each coordinate, get a dict of reads
                    # and corresponding blat hits
                    blatHitsDict = i_blatCoordinateDict[vcfKey]["rnaTumor"]
            ...
            for (readId, blatHitList) in blatHitsDict.iteritems():
                ...
                if (i_blatOutputFormat == "PSL"):
                    (isValidRead,
                     validRead) = is_valid_read_psl_format(blatHitList,
                                                           vcfChr,
                                                           vcfStopCoordinate,
                                                           i_debug)
                elif (i_blatOutputFormat == "BLAST"):
                    (isValidRead,
                     validRead) = is_valid_read_blast_format(blatHitList,
                                                             vcfChr,
                                                             vcfStopCoordinate,
                                                             0,
                                                             i_debug)
            ...
            if (numValidReads < i_readDepthCutoff or
                altPercent < i_readPercentCutoff):
        ...
        output = [vcfChr, str(vcfStopCoordinate), vcfId, vcfRef,
                  vcfAlt, vcfScore, ";".join(vcfFilterSet)]

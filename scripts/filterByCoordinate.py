...
def get_bed_data(anInputStream, anIsDebug):
    ...
    ' that will only be used as extra information if appropriate.
    '
...
def get_vcf_data(anInputFileHandler, anOutputFileHandler, aHeaderLine, anIsDebug):
    ...
    for line in anInputFileHandler:
        ...
        # now we are to the data
            ...
            yield chrom, (stopCoordinate-1), stopCoordinate, ids, ref, alt, qual, filters, info, restLine, line
...
def filter_events(aTCGAId, aChrom, aBedFilename, aVCFFilename, anOutputFilename, aFilterName, aFilterField, anIncludeOverlapInfo, anIncludeFilterName, anIncludeIdName, aFilterHeaderLine, anIsDebug):
    ...
    i_filterFileHandler = get_read_fileHandler(aBedFilename)
    ...
    if (overlappingEvents + nonOverlappingEvents == totalEvents):
        ...
        logging.info("FilterByCoordinate Warning: For chrom %s and Id %s: %s (overlapping events) + %s (non-overlapping events) = %s", aChrom, aTCGAId, overlappingEvents, nonOverlappingEvents, totalEvents)
...
def main():
    ...
    #python filterByCoordinate.py TCGA-AB-2995 12 ../data/test/filterBlacklist.bed ../data/test/TCGA-AB-2995.vcf blck -d FILTER --includeOverlaps --includeFilterName --log=DEBUG -f "##FILTER=<ID=blck,Description=\"Position overlaps 1000 Genomes Project blacklist\">"
    #python filterByCoordinate.py TCGA-AB-2995 12 ../data/test/filterSNP.bed ../data/test/TCGA-AB-2995.vcf DB -d INFO --includeOverlaps --includeFilterName --includeIdName --log=DEBUG -f "##INFO=<ID=DB,Number=0,Type=Flag,Description=\"dbSNP membership, build 135\">"
    ...
    i_cmdLineParser.add_option("-p", "--includeOverlaps", action="store_true", default=False, dest="includeOverlaps", help="whether the variants that overlap should be considered to PASS (True) or FILTER (False), %default by default")
    ...
    i_possibleArgLengths = range(5,25,1)
